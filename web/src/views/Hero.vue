<template>
  <div class="page-hero" v-if="model">
    ...
    <div class="top pt-6" :style="{'background-image': `url(${model.banner})`}">
      <div class="info text-white p-3 h-100 d-flex flex-column " >
        <div class="fs-sm text-left">{{model.title}}</div>
        <h2 class="my-2 text-left">{{model.name}}</h2>
        <!-- 遍历数组返回name，得到一个数组包含英雄所对应的的类型，换成斜杠“/”连接 -->
        <div class="fs-sm text-left">{{model.categories.map(v => v.name).join('/')}}</div>
            ...
            <div class="p-3 bg-white border-bottom">
              ...
              <div class="skills bg-white mt-4">
                ...
                <div v-if="currentSkill">
                  ...
                  <p class="text-grey-1 pt-2">小提示: {{currentSkill.tips}}</p>
                </div>
              </div>
            </div>
